...
| FAZ 08 | completed | PR candidate; no-fabrication crawl-economy evaluator; production logs/discovery lag `SKIP_NO_DATA`; Google reverse+forward DNS verification; OAI-SearchBot/OAI-AdsBot/Perplexity official-manifest CIDR verification; UA-only GPTBot/ClaudeBot remain unverified; IPv4/IPv6 CIDR coverage; INV-8.3 spoof fixture; `data/seo/crawl_economy.json`; `data/seo/slo_history.json`; `data/seo/invariant-results/faz-08.json` |
| FAZ 09 | pending | — |
| FAZ 10 | pending | — |
| FAZ 11 | pending | — |
| FAZ 12 | pending | — |
...
Measurement debt: GSC/GA4/CrUX field reporting and production request-log access remain unavailable; E-35 requires measurement-dependent fields to stay `SKIP_NO_DATA`, `partial: true`, `confidence: low`, and `coldStart: null` until measured.
