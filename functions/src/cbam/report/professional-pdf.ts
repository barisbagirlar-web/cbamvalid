...
export function buildProfessionalPdf(input: ProfessionalPdfInput): Buffer {
  ...
  const drawPageFrame = () => {
    ...
    document.text(`Package ${input.model.packageCode} | Release ${input.model.releaseVersion}`, MARGIN, 25);
  ...
  for (let pageNumber = 1; pageNumber <= pageCount; pageNumber += 1) {
    ...
    document.text(`${companyShort} | ${input.model.packageCode}`, MARGIN, 288);
