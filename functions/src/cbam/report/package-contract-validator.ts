...
import { PACKAGE_COMPONENTS } from "./package-manifest";
...
export async function validatePackageContract(
  ...
  const physicalTopLevelComponents = new Set<string>();
  for (const name of fileNames) {
  ...
  const expectedFilenames = PACKAGE_COMPONENTS.map((c) => c.filename);
  ...
  for (const top of physicalTopLevelComponents) {
    if (!(expectedFilenames as string[]).includes(top)) {
  ...
  if (parsedManifest) {
    ...
    const manifestFilenames = manifestFiles.map((f: any) => f.filename);
