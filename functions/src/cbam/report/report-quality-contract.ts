...
function hasAcceptedDecision(caseData: AuditReadyCase, topic: string): boolean {
...
export function assessVerifierGradeReport(params: {
  ...
  if (!String(caseData.installation.name.value || "").trim()) {
    add("REPORT_INSTALLATION_IDENTITY_MISSING", "BLOCKER", "Installation identity is missing.", "Enter the installation name and country.");
  }
  ...
  const supportedFields = evidenceRequirements.filter((requirement) =>
    evidenceSupports(caseData, requirement.path, requirement.evidenceId)
  ).length;
  const evidencePercentage = evidenceRequirements.length === 0
    ? 0
    : Math.round((supportedFields / evidenceRequirements.length) * 100);
  if (supportedFields !== evidenceRequirements.length) {
    add(
      "REPORT_EVIDENCE_COVERAGE_INCOMPLETE",
      "BLOCKER",
      `Only ${supportedFields} of ${evidenceRequirements.length} material fields have approved supporting evidence.`,
      "Upload, hash, approve and link evidence for every material identity, quantity, emissions, factor, precursor and allocation field."
    );
  }
  ...
  if (unresolvedQc.length > 0) {
    add(
      "REPORT_QC_NOT_CLOSED",
      "BLOCKER",
      `${unresolvedQc.length} quality-control findings remain unresolved.`,
      "Resolve all blockers and warnings before sealing the verifier-preparation package."
    );
  }
  ...
  const outputWarnings = traceNodes.flatMap((node) => node.warnings || []).filter(Boolean);
  if (outputWarnings.length > 0) {
    add("REPORT_CALCULATION_WARNINGS_OPEN", "BLOCKER", `${outputWarnings.length} calculation warnings remain in the sealed result.`, "Resolve the data or methodology conditions that produced calculation warnings.");
  }
  ...
  if (caseData.goods.length > 1) {
    ...
    if (shares.some((share) => share === null || share!.lte(0) || share!.gt(1))) {
      ...
      if (!allocationReconciled) {
        add("REPORT_ALLOCATION_NOT_RECONCILED", "BLOCKER", `Good allocation shares sum to ${sum.toString()} instead of 1.`, "Adjust allocation shares so that they reconcile exactly to 1 within a tolerance of 0.000001.");
      }
    }
    if (!hasAcceptedDecision(caseData, "GOODS_EMISSIONS_ALLOCATION")) {
      add("REPORT_ALLOCATION_METHOD_MISSING", "BLOCKER", "The multi-good emissions-allocation method is not documented and accepted.", "Add an accepted GOODS_EMISSIONS_ALLOCATION methodology decision with basis, rationale and evidence references.");
    }
  }
  ...
  for (const datum of [caseData.directEmissions, caseData.electricityConsumed, caseData.gridEmissionFactor]) {
    if (datum.sourceType === "ESTIMATED" && !hasAcceptedDecision(caseData, `ESTIMATE:${datum.id || datum.documentReference || "MATERIAL_INPUT"}`)) {
      add("REPORT_ESTIMATE_METHOD_UNDOCUMENTED", "BLOCKER", "A material input uses an estimate without an accepted methodology decision.", "Document the estimate method, legal or technical basis, uncertainty and evidence references.");
      break;
    }
  }

  const openMaterialFindings = caseData.gapAssessment.filter((gap) =>
    gap.resolutionStatus !== "RESOLVED" &&
    (gap.isBlocking || ["BLOCKER", "CRITICAL", "MAJOR"].includes(gap.severity))
  );
  ...
  return {
    ...
    issues,
    evidenceCoverage: {
      requiredFields: evidenceRequirements.length,
      supportedFields,
      percentage: evidencePercentage,
    },
    calculationIntegrity: {
      traceNodeCount: traceNodes.length,
      hashCoveragePercentage,
