...
export const REQUIRED_TOP_LEVEL_COMPONENTS = PACKAGE_COMPONENTS.map((c) => c.filename);
...
function methodologyText(caseData: AuditReadyCase, topic: string, fallback = "Not documented"): string {
...
function wrapText(doc: jsPDF, value: string, maxWidth: number): string[] {
...
function renderPdf(params: {
  ...
  const metadataRows: Array<[string, string]> = [
    ...
    ["Report standard", REPORT_STANDARD_VERSION],
  ];
    ...
    if (section.table) {
      ...
      const drawRow = (cells: string[], header: boolean) => {
        ...
        cells.forEach((_, index) => {
          ...
          if (header) doc.setFillColor(232, 237, 243);
...
export type PackageBuildContext = {
  releaseId: string;
  ...
  evidenceRows: string[][];
  fieldRows: string[][];
  materialDatums: Array<[string, InputDatum | undefined]>;
  common: {
    releaseId: string;
    caseId: string;
    caseVersion: number;
...
) => Promise<ProducedComponent | ProducedComponent[]> | ProducedComponent | ProducedComponent[];

const COMPONENT_PRODUCERS: Record<PackageComponentId, ComponentProducer> = {
  ...
  requiredDataChecklist: (ctx) => {
    const reportQualityAssessment = assessVerifierGradeReport({ caseData: ctx.caseData, calculation: ctx.calculation, qualityControls: ctx.qualityControls });
    return {
  ...
  sourceStreamRegister: (ctx) => {
    return {
      ...
      content: csv(
        ["source_stream", "value", "unit", "source_type", "evidence_id", "document_reference", "measurement_method", "responsible_person"],
        [["direct_emissions", datumText(ctx.caseData.directEmissions), ctx.caseData.directEmissions.canonicalUnit || "tCO2e", ctx.caseData.directEmissions.sourceType, ctx.caseData.directEmissions.evidenceId || "", ctx.caseData.directEmissions.documentReference || "", ctx.caseData.directEmissions.measurementMethod || "", ctx.caseData.directEmissions.responsiblePerson || ""]]
      ),
  ...
  emissionSourceRegister: (ctx) => {
    return {
      ...
      content: csv(
        ["source", "value", "unit", "factor", "factor_unit", "calculated_emissions", "evidence_status"],
        [["installation_direct", datumText(ctx.caseData.directEmissions), ctx.caseData.directEmissions.canonicalUnit || "tCO2e", "", "", ctx.calculation.installationDirectEmissions, evidenceStatus(ctx.caseData, ctx.caseData.directEmissions)], ["purchased_electricity", datumText(ctx.caseData.electricityConsumed), ctx.caseData.electricityConsumed.canonicalUnit || "MWh", datumText(ctx.caseData.gridEmissionFactor), ctx.caseData.gridEmissionFactor.canonicalUnit || "tCO2e/MWh", ctx.calculation.electricityIndirectEmissions, `${evidenceStatus(ctx.caseData, ctx.caseData.electricityConsumed)}; ${evidenceStatus(ctx.caseData, ctx.caseData.gridEmissionFactor)}`]]
      ),
  ...
  measurementMeterRegister: (ctx) => {
    return {
      ...
      content: csv(
        ["field", "measurement_method", "responsible_person", "raw_unit", "canonical_unit", "source_type", "evidence_id"],
        ctx.materialDatums.map(([path, datum]) => [path, datum?.measurementMethod || "", datum?.responsiblePerson || "", datum?.rawUnit || datum?.unit || "", datum?.canonicalUnit || "", datum?.sourceType || "", datum?.evidenceId || ""])
      ),
  ...
  activityDataLedger: (ctx) => {
    return {
      ...
      content: csv(
        ["field", "entered_value", "raw_unit", "canonical_unit", "source_type", "reporting_period", "evidence_id", "document_reference", "confidence"],
        ctx.materialDatums.map(([path, datum]) => [path, datumText(datum), datum?.rawUnit || datum?.unit || "", datum?.canonicalUnit || "", datum?.sourceType || "", datum?.reportingPeriod || ctx.facts.year, datum?.evidenceId || "", datum?.documentReference || "", datum?.confidenceStatus || ""])
      ),
  ...
  evidenceRegister: (ctx) => {
    return {
      ...
      content: csv(
        ["evidence_id", "document_type", "file_name", "issuer", "issue_date", "review_status", "support_status", "sha256", "linked_inputs", "linked_calculations"],
        ctx.evidenceRows
      ),
  ...
  methodologyDecisionLog: (ctx) => {
    return {
      ...
      content: renderPdf({
        ...
        engineVersion: ctx.common.engineVersion,
        deterministicDate: ctx.deterministicDate,
        sections: ctx.caseData.methodologyDecisions.map<PdfSection>((decision) => ({
    ...
    return {
      ...
      content: renderPdf({
        ...
        sections: [
          ...
          ...ctx.calculation.trace.map<PdfSection>((trace) => ({ heading: trace.formulaId, keyValues: [["Inputs", JSON.stringify(trace.inputs)], ["Conversions", JSON.stringify(trace.conversions || {})], ["Intermediate calculations", JSON.stringify(trace.intermediateCalculations || {})], ["Output", `${trace.outputValue} ${trace.outputUnit}`], ["Assumptions", trace.assumptions.join("; ") || "None"], ["Warnings", trace.warnings.join("; ") || "None"], ["Node hash", trace.calculationHash]] })),
        ],
      }),
  ...
  misstatementRegister: (ctx) => {
    ...
    return {
      ...
      content: csv(
        ["finding_id", "issue_type", "requirement", "severity", "affected_result", "why_it_matters", "blocking", "resolution_status", "closure_note"],
        rows
      ),
  ...
  correctiveActionLog: (ctx) => {
    ...
    return {
      ...
      content: csv(
        ["finding_id", "suggested_action", "responsible_party", "deadline", "resolution_status", "resolution_evidence_ids", "closure_note"],
        rows
      ),
  ...
  o3ciFieldMapping: (ctx) => {
    return {
      ...
      content: csv(
        ["field_group", "mapped_field", "value", "unit", "evidence_id", "calculation_or_method_reference"],
        [
          ...
          ...ctx.calculation.perGoodResults.map((result) => ["goods", `CN_${result.cnCode}`, result.specificEmbeddedEmissions, "tCO2e/t", ctx.caseData.goods[result.goodIndex]?.productionVolume.evidenceId || "", result.traceCalculationId]),
  ...
  supportingEvidence: async (ctx) => {
    const produced: ProducedComponent[] = [];
  ...
  readMeAndVerifierNavigationGuide: (ctx) => {
    ...
    const indexRows = PACKAGE_COMPONENTS.map(c => [c.filename.slice(0, 2), c.filename.slice(3), c.kind]);
    return {
...
function getPeriodEnd(caseData: AuditReadyCase): { date: Date; isoString: string } {
...
export async function buildVerifierPreparationPackage(params: {
  ...
  const materialDatums: Array<[string, InputDatum | undefined]> = [
  ...
  caseData.goods.forEach((good, index) => {
    materialDatums.push(
      [`goods.${index}.cnCode`, good.cnCode],
      [`goods.${index}.productionVolume`, good.productionVolume],
      [`goods.${index}.allocationShare`, good.allocationShare]
    );
  });
  caseData.precursors.forEach((precursor, index) => {
    materialDatums.push(
      [`precursors.${index}.quantity`, precursor.quantity],
      [`precursors.${index}.directEmissions`, precursor.directEmissions],
      [`precursors.${index}.indirectEmissions`, precursor.indirectEmissions]
    );
  });
  const fieldRows = materialDatums.map(([path, datum]) => [path, datumText(datum), datum?.canonicalUnit || datum?.unit || datum?.rawUnit || "", datum?.sourceType || "", datum?.evidenceId || "", evidenceStatus(caseData, datum), datum?.documentReference || ""]);
  ...
  const zip = new JSZip();
  const filesForManifest: ManifestFile[] = [];

  const addFile = (filename: string, content: Buffer | string, documentType: string) => {
    const buffer = Buffer.isBuffer(content) ? content : Buffer.from(content, "utf8");
  ...
  for (const comp of PACKAGE_COMPONENTS) {
  ...
  // Generate manifest
  const manifestBase: Omit<DataIntegrityManifest, "files"> & { files: ManifestFile[] } = {
    manifestVersion: "2.0",
    product: "CBAMValid Exporter Verification Preparation Pack",
    reportStandardVersion: REPORT_STANDARD_VERSION,
    releaseId,
    ...
    topLevelComponentCount: PACKAGE_COMPONENTS.length,
  ...
  const manifestDraft = { ...manifestBase, files: filesForManifest } as DataIntegrityManifest;
  ...
  const manifest: DataIntegrityManifest = { ...manifestBase, files: filesForManifest };
