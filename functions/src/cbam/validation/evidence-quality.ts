/**
 * Structured evidence quality and independent-verifiability assessment.
 *
 * A-E quality and independent verifiability are deliberately separate axes:
 * grade describes documentary authority/support quality; verifiability records
 * whether another reviewer can corroborate the file using immutable metadata,
 * structured authority provenance and external credential/reference data.
 */
  ...
  switch (documentAuthority) {
    ...
    case "INDEPENDENT":
      if (INDEPENDENT_AUTHORITY_CATEGORIES.has(issuerCategory)) {
        return { grade: "A", basis: "OFFICIAL_AUTHORITY_DOCUMENT", reasons: ["OFFICIAL_AUTHORITY_DOCUMENT"] };
      }
      if (issuerCategory === "OPERATOR_CONTROLLED") {
        return { grade: "B", basis: "OPERATOR_PRIMARY_RECORD_WITH_INTERNAL_CONTROL", reasons: ["OPERATOR_CONTROLLED_RECORD"] };
      }
...
export function assessEvidenceVerifiability(input: EvidenceVerifiabilityInput): EvidenceVerifiabilityAssessment {
  ...
  if (metadataIntegrity && independentAuthority && externalCredentialReference !== "NOT_RECORDED") {
    state = "INDEPENDENTLY_VERIFIABLE";
  } else if (metadataIntegrity && structuredAuthority && serverAssessment) {
    state = "STRUCTURALLY_VERIFIABLE";
