...
export function runQualityControls(caseData: AuditReadyCase): QualityControlResult[] {
  ...
  if (!exporterName || !installationName || !installationCountry || !productionRoute || !systemBoundaries) {
    add(
      ...
      "BLOCKER",
      "Exporter/operator name, installation name, country, production route and system-boundary statement are all required.",
      "REM_COMPLETE_INSTALLATION_IDENTITY"
    );
  } else {
  ...
  if (caseData.goods.length <= 1) {
    ...
    const shares = caseData.goods.map((good, index) => ({
      index,
      share: decimal(good.allocationShare?.value),
  ...
  const invalidDecisions = caseData.methodologyDecisions.filter((decision) =>
    decision.reviewStatus !== "ACCEPTED" ||
