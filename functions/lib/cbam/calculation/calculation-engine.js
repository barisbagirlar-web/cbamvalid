...
exports.executeDeterministicCalculation = executeDeterministicCalculation;
const certificate_engine_1 = require("../engine/certificate-engine");
const applicability_engine_1 = require("../engine/applicability-engine");
/**
 * Executes deterministic, versioned CBAM calculations with auditable trace nodes.
 */
function executeDeterministicCalculation(inputs) {
    const traces = {};
    ...
    if (inputs.isVerified && carbonPricePaidCurrency > 0) {
        eligibleCertificateReduction = Math.min(certificatesBeforeReduction, Math.floor(carbonPricePaidCurrency / pricing.priceEurPerTonne));
    }
