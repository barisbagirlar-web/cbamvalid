...
function runQualityControls(caseData) {
    var _a, _b;
    const results = [];
    const addResult = (ruleId, name, status, message) => {
        results.push({ ruleId, name, status, message });
    };
    // 1. EORI format and validation
    const eori = (_a = caseData.importerIdentity.eoriNumber.value) === null || _a === void 0 ? void 0 : _a.toString();
    if (!eori) {
    ...
    if (caseData.goods.length === 0) {
        ...
        for (const good of caseData.goods) {
            ...
            const chap = (_b = good.cnCode.value) === null || _b === void 0 ? void 0 : _b.toString().substring(0, 2);
    ...
    if (hasPrecursors) {
        ...
        for (const p of caseData.precursors) {
            if (!p.directEmissions.value && p.directEmissions.value !== 0)
                complete = false;
        }
        if (!complete) {
    ...
    if (caseData.evidenceRegister.length === 0) {
        addResult("QC_07", "Missing Evidence", "CRITICAL", "Warning: The evidence register is completely empty. We will escalate this to a BLOCKER if sealing.");
    ...
    if (caseData.carbonPriceRecords.length > 0) {
        ...
        for (const rec of caseData.carbonPriceRecords) {
            if (!rec.proofOfPaymentEvidenceId)
                missingProof = true;
        }
        if (missingProof) {
    ...
    let totalVol = 0;
    for (const good of caseData.goods)
        totalVol += Number(good.productionVolume.value || 0);
