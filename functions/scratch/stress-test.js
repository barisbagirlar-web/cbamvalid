const { performDossierCalculations } = require("../build/cbam/calculator");
...
// PDF formatSpecific helper equivalent
function formatSpecific(val) {
  if (val === null || val === undefined || String(val).trim() === "" || isNaN(Number(val))) return "N/A";
  return Number(val).toFixed(4);
}

console.log("=== RUNNING MATHEMATICAL STRESS TEST (COMMONJS) ===");
try {
  ...
  console.log("Specific Direct Emissions (Formatted):", formatSpecific(result.specificDirectEmissions));
  console.log("Specific Indirect Emissions (Formatted):", formatSpecific(result.specificIndirectEmissions));
  console.log("Specific Embedded Emissions (Formatted):", formatSpecific(result.specificEmbeddedEmissions));
  ...
  if (formatSpecific(result.specificDirectEmissions).split(".")[1]?.length === 4 &&
      formatSpecific(result.specificIndirectEmissions).split(".")[1]?.length === 4) {
