...
export function generateExecutiveDossierPdf(caseData: AuditReadyCase): Blob {
  const doc = new jsPDF({
  ...
  const addLine = (text: string, size: number, isBold: boolean = false, advance: number = 8) => {
    if (cursorY > 280) {
      doc.addPage();
      cursorY = 20;
    }
  ...
  addLine(`Sector: ${caseData.goods[0]?.sector || 'Unknown'}`, 12, false, 8);
  addLine(`CN Code Assessed: ${caseData.goods[0]?.cnCode.value}`, 12, false, 12);
...
export function generateTechnicalAnnexPdf(caseData: AuditReadyCase): Blob {
  const doc = new jsPDF({
  ...
  const calc = performDossierCalculations(caseData);
