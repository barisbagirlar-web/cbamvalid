...
export default function CasePage({ params }: { params: Promise<{ caseId: string }> }) {
  ...
  const loadWorkspace = useCallback(async () => {
    ...
    if (!isCaseId(caseId)) {
      setError("The case link is malformed. Open the dossier again from Cases.");
      setDataLoading(false);
      return;
    }

    const [caseResult, entitlementResult] = await Promise.allSettled([
      getCase(caseId),
      getEntitlements(),
    ]);
  ...
  useEffect(() => {
    ...
    void loadWorkspace();
  }, [attempt, loadWorkspace, loading, user]);
        ...
        <section className="mx-auto max-w-xl rounded-2xl border border-red-300 bg-surface p-8 shadow-sm">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-3 text-sm leading-relaxed text-muted">{error || "The case response was empty."}</p>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            ...
            <Link href="/cases" className="inline-flex h-11 items-center justify-center rounded-md border border-border bg-surface px-5 text-sm font-semibold hover:bg-neutral-soft">
              Back to Cases
            </Link>
          </div>
        </section>
  ...
  return (
    <>
      {entitlementWarning && (
        <div role="status" className="mx-auto mt-6 max-w-6xl rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          {entitlementWarning}
        </div>
      )}
      <CaseWizardClient
