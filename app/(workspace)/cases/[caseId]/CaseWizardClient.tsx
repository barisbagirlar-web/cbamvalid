...
export default function CaseWizardClient({
  ...
  const sealRequest = useRef<{ id: string; fingerprint: string } | null>(null);
  ...
  const usableEntitlements = useMemo(
    () => availableEntitlements
      .filter((entitlement) =>
        ...
        (!entitlement.scopeCaseId || entitlement.scopeCaseId === caseData.caseId)
      )
      .sort((left, right) => {
        const leftLocked = left.scopeCaseId === caseData.caseId ? 0 : 1;
        const rightLocked = right.scopeCaseId === caseData.caseId ? 0 : 1;
        return leftLocked - rightLocked || left.entitlementId.localeCompare(right.entitlementId);
      }),
    [availableEntitlements, caseData.caseId]
  );

  const selectedEntitlement = usableEntitlements[0] || null;
  const nextReleaseVersion = selectedEntitlement
    ? selectedEntitlement.releasesCount + 1
    : null;
  const correctionRequired = Boolean(nextReleaseVersion && nextReleaseVersion > 1);
  ...
  const removeGood = (index: number) => {
    setCaseData((previous) => ({
      ...previous,
      goods: previous.goods.filter((_, itemIndex) => itemIndex !== index),
    }));
  };
    ...
    setCaseData((previous) => ({
      ...
      methodologyDecisions: [
        ...previous.methodologyDecisions,
        {
          ...
          topic,
          selectedMethod: topic === "PRECURSOR_SCOPE"
            ? "No applicable precursors identified"
            : "Documented operator method",
  ...
  const linkEvidenceToDatum = (
    data: AuditReadyCase,
    path: string,
    evidenceId: string
  ): AuditReadyCase => setAtPath(data, path, (current) => ({
    ...(current as InputDatum),
    evidenceId,
  }));
  ...
  const handleEvidenceUpload = async () => {
    ...
    try {
      ...
      setEvidenceStatus(
        "Evidence uploaded as PENDING. An external malware scan and owner internal support review are still required."
      );
    } catch (error) {
  ...
  const handleEvidenceReview = async (
    ...
    setEvidenceStatus("");
    try {
      const supportStatus: EvidenceSupportStatus = decision === "APPROVED"
        ? "SUPPORTED"
        : "UNSUPPORTED";
      const updated = await reviewEvidence({
        ...
        decision,
        supportStatus,
        reviewerNotes: reviewNotes[evidenceId] || "Owner internal evidence-support review completed.",
      });
      setCaseData(updated);
      setEvidenceStatus(
        `Owner internal review marked the evidence ${decision.toLowerCase()}. This is not independent verifier approval.`
      );
    } catch (error) {
  ...
  const handleSeal = async () => {
    ...
    if (correctionRequired && normalizedReason.length < 10) {
      setSealStatus("Describe the correction in at least 10 characters before creating version 2–5.");
      return;
    }
    ...
    if (!sealRequest.current || sealRequest.current.fingerprint !== fingerprint) {
      sealRequest.current = { id: crypto.randomUUID(), fingerprint };
    }
    ...
    try {
      ...
      const response = await sealReport(
        ...
        sealRequest.current.id,
        correctionRequired ? normalizedReason : undefined
      );
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 rounded-xl border border-border bg-surface p-6 md:grid-cols-2">
        {[
          ...
          const datum = path.split(".").reduce<unknown>(
          ...
          return (
            <div key={path}>
              ...
              <input
                aria-label={label}
                type={type}
                value={datumValue(datum.value)}
                onChange={(event) => updateDatum(path, { value: event.target.value })}
  ...
  const renderStep2 = () => (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={addGood} className="inline-flex items-center gap-2 rounded bg-accent px-4 py-2 text-sm font-semibold text-surface"><Plus className="h-4 w-4" />Add good</button>
      </div>
  ...
  const emissionInput = (
    ...
    return (
      <div className="grid gap-4 rounded-xl border border-border bg-surface p-6 md:grid-cols-3">
  ...
  const renderStep7 = () => (
    <div className="space-y-8">
      <div className="flex items-center justify-between"><h2 className="text-xl font-bold">7. Carbon price and evidence register</h2><button type="button" onClick={addCarbonPriceRecord} className="inline-flex items-center gap-2 rounded border border-border px-4 py-2 text-sm"><Plus className="h-4 w-4" />Add carbon-price record</button></div>
      {caseData.carbonPriceRecords.map((record, index) => (
        <div key={record.id} className="grid gap-4 rounded-xl border border-border bg-surface p-5 md:grid-cols-2">
      ...
      <div className="space-y-4 rounded-xl border border-border bg-surface p-6">
        <h3 className="font-bold">Upload immutable evidence</h3>
        ...
        <button type="button" onClick={handleEvidenceUpload} disabled={uploading} className="inline-flex items-center gap-2 rounded bg-accent px-4 py-2 text-sm font-semibold text-surface disabled:opacity-50">{uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileUp className="h-4 w-4" />}Upload and register evidence</button>
        <StatusBanner status={evidenceStatus} tone={evidenceStatus.toLowerCase().includes("failed") || evidenceStatus.includes("EVIDENCE_") ? "error" : "warning"} />
      </div>
      ...
      <div className="space-y-3">
        {caseData.evidenceRegister.map((evidence) => (
          <div key={evidence.evidenceId} className="rounded-xl border border-border bg-surface p-4">
            <div><p className="font-semibold">{evidence.fileName}</p><p className="text-xs text-muted">{evidence.documentType} · {evidence.sizeBytes} bytes · {evidence.reviewStatus}/{evidence.supportStatus}/{evidence.malwareScanStatus}</p><p className="mt-1 break-all font-mono text-[10px] text-muted">SHA-256 {evidence.fileHash}</p></div>
            <div className="mt-3 flex flex-col gap-2 md:flex-row">
              <input aria-label={`Review notes for ${evidence.fileName}`} value={reviewNotes[evidence.evidenceId] || ""} onChange={(event) => setReviewNotes((previous) => ({ ...previous, [evidence.evidenceId]: event.target.value }))} placeholder="Owner internal support-review note" className="flex-1 rounded border border-border bg-background p-2 text-sm" />
              <button type="button" disabled={evidence.malwareScanStatus !== "CLEAN"} onClick={() => handleEvidenceReview(evidence.evidenceId, "APPROVED")} className="rounded bg-emerald-700 px-3 py-2 text-xs font-semibold text-white disabled:opacity-40">Mark internally supported</button>
              <button type="button" onClick={() => handleEvidenceReview(evidence.evidenceId, "REJECTED")} className="rounded bg-red-700 px-3 py-2 text-xs font-semibold text-white">Mark unsupported</button>
            </div>
            <p className="mt-2 text-xs text-muted">Owner internal review is a dossier-preparation control, not independent verifier approval.</p>
            {evidence.malwareScanStatus !== "CLEAN" && <p className="mt-2 text-xs text-amber-800">Internal support marking is disabled until the owner super-admin records an external malware scan as CLEAN.</p>}
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-xl border border-border bg-surface p-6">
          ...
          <div className={`rounded border p-3 text-sm font-semibold ${readiness.isEligibleForSealing ? "border-emerald-300 bg-emerald-50 text-emerald-900" : "border-red-300 bg-red-50 text-red-900"}`}>{readiness.status} · {readiness.completenessPercentage}% · {readiness.passedControls}/{readiness.applicableControls} controls passed</div>
          ...
          {selectedEntitlement ? (
            <div className="mt-5 rounded-lg border border-accent/20 bg-accent/5 p-4 text-sm">
              <p className="font-bold text-accent">Preparation Pack ready</p>
              <p className="mt-1">Next version: {nextReleaseVersion} of {selectedEntitlement.maxReleases}</p>
              <p>{selectedEntitlement.releasesRemaining} successful seal{selectedEntitlement.releasesRemaining === 1 ? "" : "s"} remaining</p>
              <p>{PREPARATION_PACK.creditsPerRelease} credits will be debited only after successful activation.</p>
            </div>
          ) : (
            <div className="mt-5 rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950">
              <p className="font-bold">No compatible Preparation Pack</p>
              <p className="mt-1">Purchase a pack or use a pack already scope-locked to this case.</p>
              <Link href="/credits/buy" className="mt-3 inline-flex items-center gap-2 font-semibold text-accent underline"><ShoppingBag className="h-4 w-4" />Purchase pack</Link>
            </div>
          )}

          {correctionRequired && (
            <div className="mt-5">
              <FieldLabel>Correction reason for release version {nextReleaseVersion}</FieldLabel>
              <textarea aria-label="Correction reason" value={correctionReason} onChange={(event) => setCorrectionReason(event.target.value)} minLength={10} maxLength={2000} rows={4} placeholder="Describe what changed since the previous sealed version and why the corrected release is required." className="w-full rounded border border-border bg-background p-3 text-sm" />
              <p className="mt-1 text-xs text-muted">Required for versions 2–5 and recorded in the immutable release history.</p>
            </div>
          )}

          <button type="button" aria-label="Generate sealed dossier" onClick={handleSeal} disabled={sealing || !readiness.isEligibleForSealing || !selectedEntitlement || (correctionRequired && correctionReason.trim().length < 10)} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded bg-accent p-3 text-sm font-semibold text-surface disabled:cursor-not-allowed disabled:opacity-40">{sealing ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}Generate sealed dossier</button>
          <StatusBanner status={sealStatus} tone="error" />
          <p className="mt-3 text-xs text-muted">A successful release is consumed only after server validation, verified immutable artifact commit, KMS signature and atomic report activation.</p>
        </section>

        <section className="rounded-xl border border-border bg-surface p-6">
          <h3 className="mb-4 flex items-center gap-2 font-bold"><FileCode2 className="h-5 w-5 text-accent" />Mathematical audit preview</h3>
          {calculation.error ? <StatusBanner status={calculation.error} tone="warning" /> : <>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><span className="text-muted">Total embedded</span><strong className="block">{calculation.result?.totalEmbeddedEmissions} tCO2e</strong></div>
            ...
            <div className="mt-4 max-h-80 space-y-3 overflow-y-auto">{calculation.result?.trace.map((trace) => <div key={trace.calculationId} className="rounded border border-border bg-neutral-soft p-3 font-mono text-xs"><div className="font-bold text-accent">{trace.formulaId}</div><div>{String(trace.outputValue)} {trace.outputUnit}</div><div className="break-all text-[10px] text-muted">{trace.calculationHash}</div></div>)}</div>
          </>}
        </section>
      </div>
    </div>
    ...
    <main className="min-h-screen bg-background px-4 py-8 pb-32 text-foreground md:px-8">
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="flex flex-col justify-between gap-4 border-b border-border pb-4 md:flex-row md:items-center">
          <div><h1 className="text-2xl font-bold">Case workflow</h1><p className="text-sm text-muted">ID: {caseData.caseId || "UNASSIGNED"} · User: {sessionUser.email || sessionUser.uid}</p></div>
          <button type="button" onClick={handleSave} disabled={saving} className="inline-flex items-center justify-center gap-2 rounded border border-border bg-neutral-soft px-4 py-2 text-sm font-medium disabled:opacity-50">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{saving ? "Saving…" : "Save draft"}</button>
        </header>
        <StatusBanner status={saveStatus} tone={saveTone} />
        <nav aria-label="Dossier steps" className="flex gap-3 overflow-x-auto pb-3">{STEPS.map((step) => <button type="button" key={step.id} onClick={() => setCurrentStep(step.id)} className={`min-w-28 rounded-lg border px-3 py-2 text-xs font-bold ${currentStep === step.id ? "border-accent bg-accent text-surface" : "border-border bg-surface text-muted"}`}><span className="block text-sm">{step.id}</span>{step.label}</button>)}</nav>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-surface p-4 shadow-[0_-4px_8px_rgba(0,0,0,0.08)]">
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          <button type="button" onClick={() => setCurrentStep((step) => Math.max(1, step - 1))} disabled={currentStep === 1} className="inline-flex items-center gap-2 rounded border border-border px-4 py-2 disabled:opacity-40"><ArrowLeft className="h-4 w-4" />Previous</button>
          <span className="text-sm font-bold text-muted">Step {currentStep} of 8</span>
          <button type="button" onClick={() => setCurrentStep((step) => Math.min(8, step + 1))} disabled={currentStep === 8} className="inline-flex items-center gap-2 rounded bg-accent px-4 py-2 text-surface disabled:opacity-40">Next<ArrowRight className="h-4 w-4" /></button>
        </div>
      </div>
    </main>
  );
