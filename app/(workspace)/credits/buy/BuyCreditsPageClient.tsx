...
const MISSING_PADDLE_TOKEN_MESSAGE =
  "Payment client token is missing in this deployment. Refresh once; if it persists, contact info@cbamvalid.com.";

export default function BuyCreditsPageClient() {
  const { user, loading } = useAuth();
  const router = useRouter();
  const searchParams = useSearchParams();
  const [paddle, setPaddle] = useState<Paddle | null>(null);
  const [loadingPkg, setLoadingPkg] = useState<string | null>(null);
  const [error, setError] = useState(PADDLE_CLIENT_TOKEN ? "" : MISSING_PADDLE_TOKEN_MESSAGE);
  ...
  // Must use useSearchParams — SSR useMemo(window) sticks as "" and blocks checkout.
  ...
  useEffect(() => {
    ...
    initializePaddle({
      ...
      .then((paddleInstance) => {
        ...
        setError(
          "Payment checkout failed to initialize. Disable browser page-translate extensions for this site, allow third-party cookies for paddle.com, then refresh."
        );
      })
      .catch((err) => {
        ...
        if (cancelled) return;
        setError(
          "Payment checkout could not load. Check that cbamvalid.com is an approved domain for the Paddle client token, then refresh."
        );
      });
    return () => {
      cancelled = true;
        ...
        if (!cancelled) {
          ...
          window.setTimeout(() => {
            if (!cancelled) {
              router.replace(
                paidCaseId
                  ? `/cases/${encodeURIComponent(paidCaseId)}?step=8&purchase=success`
      ...
      paddle.Update({
        eventCallback: async (event) => {
          ...
          try {
            ...
            sessionStorage.removeItem("cbam_pending_case_id");
            setFulfillmentPhase("confirmed");
            router.push(`/cases/${encodeURIComponent(caseId)}?step=8&purchase=success`);
          } catch (confirmErr: unknown) {
            const code =
              confirmErr && typeof confirmErr === "object" && "code" in confirmErr
                ? String((confirmErr as { code?: string }).code || "")
  ...
  const returnHref = caseId ? `/cases/${encodeURIComponent(caseId)}?step=8&purchase=success` : "/cbam?purchase=success";
