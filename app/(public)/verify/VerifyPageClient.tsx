...
export default function VerifyPageClient() {
  ...
  const runVerify = async (rawHash: string) => {
    ...
    try {
      ...
      if (data.commercialStatus === "PUBLIC_SAMPLE") {
        setState("PUBLIC_SAMPLE");
      } else if (data.commercialStatus === "REVOKED") {
        setState("REVOKED");
  ...
  useEffect(() => {
    ...
    const trySample = searchParams.get("try");
    const target =
      qHash && /^[a-fA-F0-9]{64}$/.test(qHash)
        ? qHash
        : trySample === "sample"
  ...
  const handleTrySample = () => {
    void runVerify(PUBLIC_SAMPLE_DOSSIER.primaryDocumentSha256);
  };
  ...
  const renderStatus = () => {
    switch (state) {
      case "VALIDATING":
        return (
          <div
            className="verify-result show validating"
            style={{
              padding: "20px",
          ...
          >
            <div>
              ...
              <h3 style={{ margin: 0, fontFamily: "var(--sans)", fontWeight: "bold" }}>
                Published sample integrity confirmed
              </h3>
            </div>
            <p style={{ margin: 0, fontSize: "0.86rem", color: "var(--ink)" }}>
              {result.disclaimer ||
                "This hash matches the gate-free public sample dossier. Fictional demonstration data only — not an accredited verification opinion."}
            </p>
      ...
      case "SERVICE_UNAVAILABLE":
        return (
          <div
            className="verify-result show error"
            style={{
              padding: "20px",
              background: "var(--err-soft)",
              color: "var(--err)",
          ...
          >
            <h3 style={{ margin: "0 0 6px 0", fontFamily: "var(--sans)", fontWeight: "bold" }}>
              Verification Failed
            </h3>
      ...
      <section className="section">
        <div className="wrap">
          <div className="section-head center reveal" style={{ marginBottom: "40px" }}>
            ...
            <p>
              Received a CBAMValid dossier from an exporter? Enter its document hash signature below
              to confirm the cryptographic seal — proof it hasn&apos;t been tampered with.
            </p>
          </div>
          ...
          <div className="verify-box reveal">
            <form onSubmit={handleVerify}>
              ...
              <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                ...
                <button className="btn btn-ghost" type="button" onClick={handleTrySample}>
                  Try it (public sample)
                </button>
                {hash && (
          ...
          <div className="method-grid" style={{ marginTop: "64px" }}>
            <div className="method-card reveal">
              ...
              <p>
                The SHA-256 hashes of the sealed PDF and JSON, the UTC seal timestamp, and the exact
                ruleset version the dossier was calculated against.
              </p>
            ...
            <div className="method-card reveal">
              ...
              <p>
                Verification proves integrity, not regulatory approval. CBAMValid is not an accredited
                verifier; legal verification of emissions remains a separate step where required.
              </p>
